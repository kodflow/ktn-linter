// Package rules_alloc_bad contient des violations KTN-ALLOC-003.
package rules_alloc_bad

// Viole KTN-ALLOC-003 : new(struct) au lieu de &struct{}
...
func BadNewUser() *User {
	u := new(User) // Viole KTN-ALLOC-003
	u.ID = 1
	u.Name = "Alice"
...
func BadNewConfig() *Config {
	cfg := new(Config) // Viole KTN-ALLOC-003
	cfg.Host = "localhost"
	cfg.Port = 8080
...
func BadNewStructInline() {
	...
	p := new(Point) // Viole KTN-ALLOC-003
	p.X = 10
	p.Y = 20
}
...
func BadNewMultipleStructs() {
	u1 := new(User)    // Viole KTN-ALLOC-003
	u2 := new(User)    // Viole KTN-ALLOC-003
	cfg := new(Config) // Viole KTN-ALLOC-003
	_, _, _ = u1, u2, cfg
}
...
func BadNewNestedStruct() {
	...
	addr := new(Address) // Viole KTN-ALLOC-003
	addr.Street = "123 Main St"
	addr.City = "Paris"

	person := new(Person) // Viole KTN-ALLOC-003
	person.Name = "Bob"
	person.Address = addr
}
...
func BadNewEmptyStruct() {
	type Empty struct{}
	e := new(Empty) // Viole KTN-ALLOC-003
	_ = e
}
...
func BadNewStructWithMethods() {
	...
	c := new(Counter) // Viole KTN-ALLOC-003
	c.count = 10
}
...
func BadNewStructAsReturn() *User {
	...
	return new(User) // Viole KTN-ALLOC-003
}
...
func BadNewStructComplex() {
	...
	db := new(Database) // Viole KTN-ALLOC-003
	db.Host = "localhost"
	db.Port = 5432
}
...
func BadNewStructPointerField() {
	...
	node := new(Node) // Viole KTN-ALLOC-003
	node.Value = 42
	node.Next = nil
}
...
func BadNewGenericStruct() {
	...
	cont := new(Container) // Viole KTN-ALLOC-003
	cont.Data = "test"
}
