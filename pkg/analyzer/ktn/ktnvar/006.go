...
// Analyzer006 checks that make with length > 0 is avoided when append is used
var Analyzer006 = &analysis.Analyzer{
	Name:     "ktnvar006",
	Doc:      "KTN-VAR-006: Vérifie d'éviter make([]T, length) si utilisation avec append",
	Run:      runVar006,
...
func checkMakeCallVar008(pass *analysis.Pass, call *ast.CallExpr) {
	...
	pass.Reportf(
		call.Pos(),
		"KTN-VAR-006: utiliser make([]T, 0, capacity) au lieu de make([]T, length) pour éviter les zéro-values inutiles avant append",
	)
}
