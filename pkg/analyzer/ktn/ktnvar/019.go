...
// Analyzer019 détecte les copies de mutex.
//
// Copier un mutex crée des bugs de concurrence car la copie ne partage pas
// le même état de verrouillage.
var Analyzer019 *analysis.Analyzer = &analysis.Analyzer{
	Name:     "ktnvar019",
	Doc:      "KTN-VAR-019: Vérifie les copies de mutex (sync.Mutex, sync.RWMutex, atomic.Value)",
	Run:      runVar019,
	Requires: []*analysis.Analyzer{inspect.Analyzer},
}

// runVar019 exécute l'analyse de détection des copies de mutex.
//
// Params:
...
func runVar019(pass *analysis.Pass) (interface{}, error) {
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		...
		for _, field := range structType.Fields.List {
			...
			if mutexType := getMutexType(pass, field.Type); mutexType != "" {
				...
				pass.Reportf(
					field.Pos(),
					"KTN-VAR-019: struct contient %s, utiliser *%s pour éviter les copies",
					mutexType,
					typeSpec.Name.Name,
				)
			}
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		...
		if !isPointerType(recv.Type) {
			...
			if hasMutex(pass, recv.Type) {
				...
				pass.Reportf(
					recv.Pos(),
					"KTN-VAR-019: receiver par valeur copie %s, utiliser *%s",
					mutexType,
					typeName,
				)
			}
		...
		for _, param := range funcDecl.Type.Params.List {
			...
			if mutexType := getMutexType(pass, param.Type); mutexType != "" {
				...
				if !isPointerType(param.Type) {
					pass.Reportf(
						param.Pos(),
						"KTN-VAR-019: passage de %s par valeur, utiliser *%s",
						mutexType,
						mutexType,
					)
				}
		...
		for i, rhs := range assign.Rhs {
			...
			if i < len(assign.Lhs) {
				...
				if isMutexCopy(pass, assign.Lhs[i], rhs) {
					pass.Reportf(
						assign.Pos(),
						"KTN-VAR-019: copie de sync.Mutex détectée, utiliser un pointeur",
					)
				}
			}
		}
