...
const (
	// ruleCodeVar019 is the rule code for this analyzer
	ruleCodeVar019 string = "KTN-VAR-019"
...
func runVar019(pass *analysis.Pass) (any, error) {
	...
	if !cfg.IsRuleEnabled(ruleCodeVar019) {
		// Règle désactivée
		return nil, nil
	}
...
func checkStructsWithMutex(pass *analysis.Pass, insp *inspector.Inspector, typesWithValueRecv map[string]bool) {
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		// Skip excluded files
		if cfg.IsFileExcluded(ruleCodeVar019, pass.Fset.Position(n.Pos()).Filename) {
			// Fichier exclu
			return
		}
