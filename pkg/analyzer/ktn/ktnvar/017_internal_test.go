...
// Test_runVar017_fileExcludedWithMap tests file exclusion with make(map) that would trigger.
func Test_runVar017_fileExcludedWithMap(t *testing.T) {
	// Setup config with file exclusion
	config.Set(&config.Config{
		Rules: map[string]*config.RuleConfig{
			"KTN-VAR-017": {
...
`
	...
	if reportCount != 0 {
		t.Errorf("runVar017() reported %d, expected 0 when file excluded", reportCount)
	}
}

// Test_runVar017_withMakeSlice tests runVar017 with make([]T) - should not trigger.
func Test_runVar017_withMakeSlice(t *testing.T) {
	config.Reset()
...
`
	...
	if reportCount != 0 {
		t.Errorf("runVar017() with slice reported %d, expected 0", reportCount)
	}
}

// Test_runVar017_withMakeChan tests runVar017 with make(chan T) - should not trigger.
func Test_runVar017_withMakeChan(t *testing.T) {
	config.Reset()
...
`
	...
	if reportCount != 0 {
		t.Errorf("runVar017() with chan reported %d, expected 0", reportCount)
	}
}
