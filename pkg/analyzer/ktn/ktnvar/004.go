...
// Analyzer004 checks that local variables use := instead of var
var Analyzer004 = &analysis.Analyzer{
	Name:     "ktnvar004",
	Doc:      "KTN-VAR-004: Vérifie que les variables locales utilisent ':=' au lieu de 'var'",
	Run:      runVar004,
...
func reportVarErrors(pass *analysis.Pass, spec *ast.ValueSpec) {
	...
	for _, name := range spec.Names {
		pass.Reportf(
			name.Pos(),
			"KTN-VAR-004: préférer ':=' au lieu de 'var' pour la variable '%s'",
			name.Name,
		)
	}
}
