...
const (
	// ruleCodeVar010 is the rule code for this analyzer
	ruleCodeVar010 string = "KTN-VAR-010"
)

// Analyzer010 checks for strings.Builder/bytes.Buffer without Grow preallocate
var Analyzer010 *analysis.Analyzer = &analysis.Analyzer{
	Name:     "ktnvar010",
	Doc:      "KTN-VAR-010: Préallouer bytes.Buffer/strings.Builder avec Grow",
	Run:      runVar010,
...
func runVar010(pass *analysis.Pass) (any, error) {
	...
	if !cfg.IsRuleEnabled(ruleCodeVar010) {
		// Règle désactivée
		return nil, nil
	}
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		// Skip excluded files
		if cfg.IsFileExcluded(ruleCodeVar010, pass.Fset.Position(n.Pos()).Filename) {
			// Fichier exclu
			return
		}
