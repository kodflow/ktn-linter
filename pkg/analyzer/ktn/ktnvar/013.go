...
// Analyzer013 détecte les conversions string() répétées.
//
...
var Analyzer013 = &analysis.Analyzer{
	Name:     "ktnvar013",
	Doc:      "KTN-VAR-013: Vérifie les conversions string() répétées",
	Run:      runVar013,
	Requires: []*analysis.Analyzer{inspect.Analyzer},
}

// runVar013 exécute l'analyse de détection des conversions répétées.
//
// Params:
...
func checkLoopsForStringConversion(pass *analysis.Pass, body *ast.BlockStmt) {
	...
	ast.Inspect(body, func(n ast.Node) bool {
		...
		if hasStringConversion(loop) {
			...
			pass.Reportf(
				loop.Pos(),
				"KTN-VAR-013: conversion string() répétée dans la boucle, préallouer hors de la boucle",
			)
		}
...
func checkMultipleConversions(pass *analysis.Pass, body *ast.BlockStmt) {
	...
	for varName, count := range conversions {
		...
		if count > MAX_ALLOWED_CONVERSIONS {
			...
			pass.Reportf(
				pos.Pos(),
				"KTN-VAR-013: conversion string() de '%s' répétée %d fois, stocker dans une variable",
				varName,
				count,
			)
		}
