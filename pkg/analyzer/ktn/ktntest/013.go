...
var (
	...
	// Analyzer013 detects passthrough tests that don't test anything.
	Analyzer013 = &analysis.Analyzer{
		Name:     "ktntest013",
		Doc:      "KTN-TEST-013: Les tests doivent contenir des assertions et vraiment tester quelque chose",
		Run:      runTest013,
...
func runTest013(pass *analysis.Pass) (any, error) {
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		...
		if isPassthroughTest(funcDecl) {
			pass.Reportf(
				funcDecl.Pos(),
				"KTN-TEST-013: le test '%s' est un test passthrough - "+
