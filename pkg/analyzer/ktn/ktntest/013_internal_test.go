...
// Test_runTest013 tests the runTest013 function indirectly via the analyzer.
//
// Params:
//   - t: testing context
func Test_runTest013(t *testing.T) {
	tests := []struct {
		...
		{"analyzer exists", "notNil", Analyzer013 != nil},
		{"run is defined", "notNil", Analyzer013.Run != nil},
		{"name is correct", "name", Analyzer013.Name},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "name":
				// Vérification du nom
				if tt.expected != "ktntest013" {
					t.Errorf("expected ktntest013, got %v", tt.expected)
				}
			}
		})
	}
