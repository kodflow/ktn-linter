...
// Analyzer008 checks private function tests are in internal test files
var Analyzer008 *analysis.Analyzer = &analysis.Analyzer{
	Name:     "ktntest008",
	Doc:      "KTN-TEST-008: Les tests de fonctions privées (non-exportées) doivent être dans _internal_test.go uniquement (white-box testing)",
	Run:      runTest008,
...
func checkAndReportPrivateFunctionTest(pass *analysis.Pass, funcDecl *ast.FuncDecl, baseName string, privateFunctions map[string]bool) {
	...
	if privateFunctions[key] {
		...
		pass.Reportf(
			funcDecl.Pos(),
			"KTN-TEST-008: le test '%s' dans '%s' teste une fonction privée '%s'. Les tests de fonctions privées doivent être dans '%s' (white-box testing avec package xxx)",
