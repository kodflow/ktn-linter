...
// Analyzer006 checks that each source file has appropriate test files based on its content
var Analyzer006 *analysis.Analyzer = &analysis.Analyzer{
	Name: "ktntest006",
	Doc:  "KTN-TEST-006: Chaque fichier .go doit avoir les fichiers de test appropriés (xxx_internal_test.go si fonctions privées, xxx_external_test.go si fonctions publiques)",
	Run:  runTest006,
}
...
func reportMixedFunctionsIssues(pass *analysis.Pass, file *ast.File, _result *fileAnalysisResult, status testFilesStatus) {
	msg, _ := messages.Get(ruleCodeTest006)
