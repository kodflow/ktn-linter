...
// Analyzer006 vérifie que les champs exportés sont avant les champs privés
var Analyzer006 = &analysis.Analyzer{
	Name:     "ktnstruct006",
	Doc:      "KTN-STRUCT-006: Les champs exportés doivent être placés avant les champs privés dans une struct",
	Run:      runStruct006,
...
func checkFieldOrder(pass *analysis.Pass, typeSpec *ast.TypeSpec, structType *ast.StructType) {
	...
	for _, f := range fields {
		...
		if f.exported && foundPrivate {
			pass.Reportf(
				f.pos,
				"KTN-STRUCT-006: le champ exporté '%s' de la struct '%s' doit être placé avant les champs privés",
				f.name,
				typeSpec.Name.Name,
			)
		}
