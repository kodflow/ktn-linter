...
// Analyzer004 checks getters don't have "Get" prefix (Go idiom)
var Analyzer004 = &analysis.Analyzer{
	Name:     "ktnstruct004",
	Doc:      "KTN-STRUCT-004: Les getters ne doivent pas avoir le préfixe 'Get' (convention Go idiomatique)",
	Run:      runStruct004,
...
func runStruct004(pass *analysis.Pass) (any, error) {
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		...
		pass.Reportf(
			funcDecl.Name.Pos(),
			"KTN-STRUCT-004: la méthode '%s' devrait être renommée '%s' (convention Go idiomatique, voir https://go.dev/doc/effective_go#Getters)",
			methodName,
			suggestedName,
		)
	})
