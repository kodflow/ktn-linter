...
// Analyzer002 vérifie que les structs exportées ont une documentation
var Analyzer002 = &analysis.Analyzer{
	Name:     "ktnstruct002",
	Doc:      "KTN-STRUCT-002: Toute struct exportée doit avoir une documentation complète",
	Run:      runStruct002,
	...
	insp.Preorder(nodeFilter, func(n ast.Node) {
		...
		for _, spec := range genDecl.Specs {
			...
			if !hasValidDocumentation(genDecl.Doc, typeSpec.Name.Name) {
				pass.Reportf(
					typeSpec.Pos(),
					"KTN-STRUCT-002: la struct exportée '%s' doit avoir une documentation complète (au moins 2 lignes décrivant son rôle)",
					typeSpec.Name.Name,
				)
			}
		}
