...
// isMakeCallVar015 vérifie si l'appel est à make.
//
// Params:
//   - call: expression d'appel
//
...
func isMakeCallVar015(call *ast.CallExpr) bool {
	ident, ok := call.Fun.(*ast.Ident)
	// Return true if it's an identifier named 'make'
	return ok && ident.Name == "make"
}

// isByteSlice vérifie si le type est []byte.
//
// Params:
...
func isByteSlice(call *ast.CallExpr) bool {
	// Need at least one argument (the type)
	if len(call.Args) == 0 {
	...
	// Return false if not an array type
	if !ok {
		// Not an array type
		return false
	}
	...
	// Return true if element is byte
