...
build: ## Compile et installe ktn-linter
	@echo "${GREEN}Compilation de ktn-linter ${VERSION}...${NC}"
	@mkdir -p builds ~/.local/bin
	...
	@echo "${GREEN}✅ ktn-linter ${VERSION} installé${NC}"

install: build ## Installe ktn-linter dans /usr/local/bin (nécessite sudo)
	@if [ -w /usr/local/bin ]; then \
		...
		echo "${YELLOW}⚠ Impossible d'écrire dans /usr/local/bin (utilisez sudo make install)${NC}"; \
	fi
